...
function parsearNumero(valor) {
    ...
    if (str.includes(",") && str.includes(".")) {
        ...
        if (ultimaComa > ultimoPunto) return parseFloat(str.replace(/\./g, "").replace(",", ".")) || 0
        else return parseFloat(str.replace(/,/g, "")) || 0
    }
    if (str.includes(",")) {
        const partes = str.split(",")
        if (partes.length === 2 && partes[1].length <= 2) return parseFloat(str.replace(",", ".")) || 0
...
function ModalNuevoIngrediente({ nombreSugerido, onGuardar, onCancelar }) {
    ...
    const labelStyle = { fontSize: 11, fontWeight: 700, color: "#718096", textTransform: "uppercase", letterSpacing: "0.5px", display: "block", marginBottom: 6 }

    return (
        <div onClick={onCancelar} style={{ position: "fixed", inset: 0, zIndex: 1000, background: "rgba(0,0,0,0.6)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
            <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 20, width: "100%", maxWidth: 520, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 25px 80px rgba(0,0,0,0.3)" }}>
                <div style={{ padding: "24px 28px", borderBottom: "2px solid #e8f8f5" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <div>
                            ...
                            <p style={{ margin: "4px 0 0", fontSize: 13, color: "#718096" }}>Agregar "{nombreSugerido}" al inventario</p>
                        </div>
                        <button onClick={onCancelar} style={{ all: "unset", cursor: "pointer", fontSize: 24, color: "#a0aec0", padding: 4 }}>✕</button>
                    </div>
                </div>
                <div style={{ padding: "24px 28px" }}>
                    <div style={{ marginBottom: 16 }}>
                        ...
                        <div style={{ display: "flex", gap: 8 }}>
                            {[{ valor: "ingrediente", label: "🧁 Ingrediente", color: "#6366f1" }, { valor: "insumo", label: "📦 Insumo", color: "#10b981" }].map(({ valor, label, color }) => (
                                <button key={valor} type="button" onClick={() => setForm(prev => ({ ...prev, tipo: valor }))} style={{ flex: 1, padding: "10px 0", borderRadius: 10, border: `2px solid ${form.tipo === valor ? color : "#e2e8f0"}`, background: form.tipo === valor ? color : "transparent", color: form.tipo === valor ? "#fff" : "#718096", cursor: "pointer", fontSize: 13, fontWeight: 600 }}>{label}</button>
                            ))}
                        </div>
                    </div>
                    <div style={{ marginBottom: 16 }}><span style={labelStyle}>Nombre</span><input type="text" value={form.nombre} onChange={e => setForm(prev => ({ ...prev, nombre: e.target.value }))} style={inputStyle} placeholder="Ej: Harina de trigo" /></div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 16 }}>
                        <div><span style={labelStyle}>Paquetes *</span><input type="text" inputMode="decimal" value={form.cantidadPaquetes} onChange={e => setForm(prev => ({ ...prev, cantidadPaquetes: e.target.value }))} style={inputStyle} placeholder="2" /></div>
                        <div><span style={labelStyle}>Cantidad/paquete *</span><input type="text" inputMode="decimal" value={form.tamañoPaquete} onChange={e => setForm(prev => ({ ...prev, tamañoPaquete: e.target.value }))} style={inputStyle} placeholder="1000" /></div>
                        <div><span style={labelStyle}>Unidad</span><select value={form.unidadPaquete} onChange={e => setForm(prev => ({ ...prev, unidadPaquete: e.target.value }))} style={inputStyle}>{UNIDADES_PAQUETE.map(u => <option key={u} value={u}>{u}</option>)}</select></div>
                    </div>
                    <div style={{ marginBottom: 16 }}>
                        <span style={labelStyle}>Costo por paquete (₡) *</span>
                        <div style={{ position: "relative" }}>
                            <span style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: "#718096", fontWeight: 600, fontSize: 14, pointerEvents: "none" }}>₡</span>
                            <input type="text" inputMode="decimal" value={form.costoPorPaquete} onChange={e => setForm(prev => ({ ...prev, costoPorPaquete: e.target.value }))} style={{ ...inputStyle, paddingLeft: 32 }} placeholder="7,140" />
                        </div>
                    ...
                    {totalInventario > 0 && costoTotal > 0 && (
                        <div style={{ background: "#e8f8f5", border: "2px solid #2ec4a9", borderRadius: 12, padding: 16, marginBottom: 20 }}>
                            <div style={{ display: "flex", justifyContent: "space-between", paddingTop: 8, borderTop: "1px solid #2ec4a9" }}>
                                ...
                                <span style={{ fontSize: 18, fontWeight: 700, color: "#1a9e87" }}>₡{costoPorUnidad}</span>
                            </div>
                        </div>
                    )}
                    <div style={{ display: "flex", gap: 10 }}>
                        <button onClick={handleGuardar} disabled={!form.nombre.trim() || !form.cantidadPaquetes || !form.tamañoPaquete || !form.costoPorPaquete} style={{ flex: 1, padding: "12px 20px", borderRadius: 10, background: (!form.nombre.trim() || !form.cantidadPaquetes || !form.tamañoPaquete || !form.costoPorPaquete) ? "#cbd5e0" : "#1a9e87", color: "#fff", border: "none", cursor: "pointer", fontSize: 14, fontWeight: 600 }}>💾 Guardar en inventario</button>
                        <button onClick={onCancelar} style={{ padding: "12px 20px", borderRadius: 10, background: "#e2e8f0", color: "#4a5568", border: "none", cursor: "pointer", fontSize: 14, fontWeight: 600 }}>Cancelar</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    return (
        <div style={{ background: "#fff", borderRadius: 16, overflow: "hidden", boxShadow: "0 2px 12px rgba(0,0,0,0.08)", transition: "transform 0.15s, box-shadow 0.15s", border: "1px solid var(--gris-borde)", position: "relative" }}
            ...
            <div style={{ position: "absolute", top: 8, right: 8, zIndex: 10 }}>
                {confirmando ? (
                    <div style={{ display: "flex", alignItems: "center", gap: 5, background: "#fff", border: "1.5px solid #fecaca", borderRadius: 10, padding: "4px 8px", boxShadow: "0 2px 8px rgba(0,0,0,0.15)" }}>
                        <span style={{ fontSize: 11, color: "#e53e3e", fontWeight: 600 }}>¿Eliminar?</span>
                        <button type="button" onClick={e => { e.stopPropagation(); onEliminar(receta) }} style={{ all: "unset", cursor: "pointer", background: "#e53e3e", color: "#fff", fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 6 }}>Sí</button>
                        <button type="button" onClick={e => { e.stopPropagation(); setConfirmando(false) }} style={{ all: "unset", cursor: "pointer", background: "#e2e8f0", color: "#2d3748", fontSize: 11, fontWeight: 600, padding: "2px 8px", borderRadius: 6 }}>No</button>
                    </div>
                ) : (
                    <button type="button" onClick={e => { e.stopPropagation(); setConfirmando(true) }} style={{ all: "unset", cursor: "pointer", background: "rgba(255,255,255,0.9)", color: "#e53e3e", width: 28, height: 28, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, boxShadow: "0 1px 4px rgba(0,0,0,0.15)" }}>🗑</button>
                )}
            </div>
            <div onClick={() => onVerDetalle(receta)} style={{ cursor: "pointer" }}>
                <div style={{ width: "100%", height: 160, background: foto ? "transparent" : "linear-gradient(135deg, var(--verde-claro), #d0f0ea)", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
                    {foto ? <img src={foto} alt={receta.nombre} style={{ width: "100%", height: "100%", objectFit: "cover" }} onError={e => { e.target.style.display = "none" }} /> : <span style={{ fontSize: 48 }}>🍩</span>}
                </div>
    ...
    return (
        <div onClick={onCerrar} style={{ position: "fixed", inset: 0, zIndex: 500, background: "rgba(0,0,0,0.55)", backdropFilter: "blur(3px)", display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "24px 16px", overflowY: "auto" }}>
            <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 18, width: "100%", maxWidth: 600, overflow: "hidden", boxShadow: "0 20px 60px rgba(0,0,0,0.25)" }}>
                {foto && <img src={foto} alt={receta.nombre} style={{ width: "100%", maxHeight: 260, objectFit: "cover" }} onError={e => { e.target.style.display = "none" }} />}
                <div style={{ padding: "20px 24px 28px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
                        <div>
                            <h3 style={{ margin: 0, fontSize: 20 }}>{receta.nombre}</h3>
                            <p style={{ color: "var(--texto-suave)", fontSize: 13, marginTop: 4 }}>{receta.categoria}{receta.unidades ? ` · ${receta.unidades} porciones` : ""}</p>
                        </div>
                        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                            <button type="button" onClick={() => onEditar(receta)} style={{ all: "unset", cursor: "pointer", fontSize: 13, fontWeight: 600, background: "var(--verde-claro)", color: "var(--verde-oscuro)", padding: "6px 12px", borderRadius: 8 }}>✏️ Editar</button>
                            {confirmando ? (
                                <div style={{ display: "flex", alignItems: "center", gap: 5, background: "#fff5f5", border: "1.5px solid #fecaca", borderRadius: 8, padding: "4px 8px" }}>
                                    <span style={{ fontSize: 12, color: "#e53e3e", fontWeight: 600 }}>¿Eliminar?</span>
                                    <button type="button" onClick={() => onEliminar(receta)} style={{ all: "unset", cursor: "pointer", background: "#e53e3e", color: "#fff", fontSize: 12, fontWeight: 700, padding: "3px 10px", borderRadius: 6 }}>Sí</button>
                                    <button type="button" onClick={() => setConfirmando(false)} style={{ all: "unset", cursor: "pointer", background: "#e2e8f0", color: "#2d3748", fontSize: 12, fontWeight: 600, padding: "3px 10px", borderRadius: 6 }}>No</button>
                                </div>
                            ) : (
                                <button type="button" onClick={() => setConfirmando(true)} style={{ all: "unset", cursor: "pointer", fontSize: 16, color: "#e53e3e", padding: "4px 8px" }}>🗑</button>
                            )}
                        ...
                        <div style={{ marginBottom: 20 }}>
                            ...
                            {receta.pasos.map((paso, i) => (
                                <div key={i} style={{ display: "flex", gap: 12, marginBottom: 10 }}>
                                    <div style={{ width: 24, height: 24, borderRadius: "50%", flexShrink: 0, background: "var(--verde-claro)", color: "var(--verde-oscuro)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 12 }}>{i + 1}</div>
                                    <p style={{ fontSize: 14, margin: 0, lineHeight: 1.6, paddingTop: 2 }}>{paso}</p>
                                </div>
                            ))}
                        </div>
                    )}
                    <button type="button" onClick={() => onImprimir([receta])} style={{ all: "unset", cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 8, background: "var(--verde-oscuro)", color: "#fff", padding: "10px 18px", borderRadius: 10, fontSize: 13, fontWeight: 600 }}>🖨️ Imprimir esta receta</button>
                </div>
            </div>
        </div>
    )
                ...
                return (
                    <div key={receta._id} className="receta-print-pagina" style={{ padding: "0 0 40px" }}>
                        <div style={{ borderBottom: "3px solid #1a9e87", paddingBottom: 12, marginBottom: 20 }}>
                            ...
                            <p style={{ margin: "4px 0 0", color: "#718096", fontSize: 13 }}>{receta.categoria}{receta.unidades ? ` · ${receta.unidades} porciones` : ""}</p>
                        </div>
                                ...
                                {(receta.ingredientes || []).length > 0 && (<>
                                    ...
                                    {receta.ingredientes.map((ing, i) => (
                                        <div key={ing.id || i} style={{ display: "flex", justifyContent: "space-between", padding: "5px 0", borderBottom: "1px solid #e2e8f0", fontSize: 13 }}>
                                            <span>{ing.nombre}</span><span style={{ color: "#718096" }}>{ing.cantidadUso} {ing.unidadUso}</span>
                                        </div>
                                    ))}
                                </>)}
                            </div>
                                ...
                                <div>
                                    ...
                                    {receta.pasos.map((paso, i) => (
                                        <div key={i} style={{ display: "flex", gap: 10, marginBottom: 10 }}>
                                            <div style={{ width: 22, height: 22, borderRadius: "50%", flexShrink: 0, background: "#e8f8f5", color: "#1a9e87", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 11 }}>{i + 1}</div>
...
function FormularioRecetario({ inicial, onGuardar, onCancelar, inventario = [], onAgregarIngredienteInventario }) {
    ...
    const seleccionarIng = (ing) => { setIngForm(prev => ({ ...prev, nombre: ing.nombre })); setBusquedaIng(""); setMostrarSugerencias(false) }

    const handleAgregarIngrediente = () => {
        if (!ingForm.nombre.trim()) return
        const existe = inventario.some(ing => ing.nombre.toLowerCase() === ingForm.nombre.trim().toLowerCase())
        if (!existe) { setModalNuevoIng({ nombre: ingForm.nombre.trim() }); return }
        agregarIngredienteALista()
    }

    const agregarIngredienteALista = () => {
        if (!ingForm.nombre.trim()) return
        setForm(prev => ({ ...prev, ingredientes: [...prev.ingredientes, { ...ingForm, id: crypto.randomUUID() }] }))
    ...
    const editarPaso = (i, val) => setForm(prev => { const pasos = [...prev.pasos]; pasos[i] = val; return { ...prev, pasos } })
        ...
        <>
            <div style={{ background: "#fff", borderRadius: 16, boxShadow: "0 2px 16px rgba(0,0,0,0.07)", padding: 24, marginBottom: 20 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                    <h3 style={{ margin: 0, fontSize: 16 }}>{inicial ? "✏️ Editar receta" : "➕ Nueva receta"}</h3>
                    <button type="button" onClick={onCancelar} style={{ all: "unset", cursor: "pointer", fontSize: 20, color: "#718096", padding: "2px 8px" }}>✕</button>
                </div>
                <div style={{ display: "flex", gap: 20, alignItems: "center", marginBottom: 16, flexWrap: "wrap", justifyContent: "center" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 180, display: "flex", flexDirection: "column", gap: 10 }}>
                        <div><span style={labelStyle}>Nombre de la receta</span><input type="text" value={form.nombre} placeholder="Ej: Minidonas San Valentín" onChange={e => set("nombre", e.target.value)} style={inputStyle} /></div>
                        <div><span style={labelStyle}>Categoría</span><input type="text" value={form.categoria} onChange={e => set("categoria", e.target.value)} style={inputStyle} /></div>
                        <div><span style={labelStyle}>Porciones</span><input type="number" value={form.unidades} placeholder="Ej: 24" onChange={e => set("unidades", e.target.value)} style={inputStyle} /></div>
                    </div>
                </div>
                    ...
                    <div style={{ position: "relative", marginBottom: 10 }}>
                        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                            <div style={{ flex: 2, minWidth: 200, position: "relative" }}>
                                <input type="text" placeholder="Buscar o escribir ingrediente..." value={busquedaIng || ingForm.nombre}
                                    onChange={e => { setBusquedaIng(e.target.value); setIngForm(prev => ({ ...prev, nombre: e.target.value })); setMostrarSugerencias(true) }}
                                    onFocus={() => setMostrarSugerencias(true)}
                                    onKeyDown={e => { if (e.key === "Enter") { e.preventDefault(); handleAgregarIngrediente() } }}
                                    style={inputStyle} />
                            ...
                            <input type="text" placeholder="Cantidad" value={ingForm.cantidadUso} onChange={e => setIngForm(prev => ({ ...prev, cantidadUso: e.target.value }))} style={{ ...inputStyle, flex: 1, minWidth: 80 }} />
                            <select value={ingForm.unidadUso} onChange={e => setIngForm(prev => ({ ...prev, unidadUso: e.target.value }))} style={{ ...inputStyle, flex: 1, minWidth: 90 }}>{UNIDADES_ING.map(u => <option key={u}>{u}</option>)}</select>
                            <button type="button" onClick={handleAgregarIngrediente} style={{ all: "unset", cursor: "pointer", background: "#1a9e87", color: "#fff", padding: "9px 16px", borderRadius: 10, fontSize: 13, fontWeight: 600, display: "flex", alignItems: "center", flexShrink: 0 }}>+ Agregar</button>
                        </div>
                        {mostrarSugerencias && <div onClick={() => setMostrarSugerencias(false)} style={{ position: "fixed", inset: 0, zIndex: 50 }} />}
                    </div>
                    {form.ingredientes.map(ing => (
                        <div key={ing.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 12px", background: "#f8fffe", borderRadius: 8, marginBottom: 5, fontSize: 14, border: "1.5px solid #e8f8f5" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 8 }}><span style={{ fontSize: 16 }}>🧁</span><span style={{ fontWeight: 500 }}>{ing.nombre}</span></div>
                            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                                <span style={{ color: "#718096", fontSize: 13, background: "#fff", padding: "2px 8px", borderRadius: 6 }}>{ing.cantidadUso} {ing.unidadUso}</span>
                                <button type="button" onClick={() => eliminarIng(ing.id)} style={{ all: "unset", cursor: "pointer", color: "#e53e3e", fontSize: 14, fontWeight: 700, width: 24, height: 24, display: "flex", alignItems: "center", justifyContent: "center", borderRadius: 6 }}>✕</button>
                            </div>
                        </div>
                    ))}
                    {form.ingredientes.length === 0 && <p style={{ fontSize: 13, color: "#a0aec0", fontStyle: "italic", textAlign: "center", padding: 20 }}>Agrega ingredientes buscando en el inventario o escribiendo uno nuevo</p>}
                </div>
                <div style={{ borderTop: "1px solid #e2e8f0", paddingTop: 16, marginBottom: 20 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                        ...
                        <button type="button" onClick={agregarPaso} className="btn-secundario" style={{ fontSize: 12, padding: "5px 12px" }}>+ Agregar paso</button>
                    </div>
                    {form.pasos.length === 0 && <p style={{ fontSize: 13, color: "#718096", fontStyle: "italic" }}>Sin pasos aún.</p>}
                    {form.pasos.map((paso, i) => (
                        <div key={i} style={{ display: "flex", gap: 8, alignItems: "flex-start", marginBottom: 8 }}>
                            <div style={{ width: 26, height: 26, borderRadius: "50%", flexShrink: 0, marginTop: 8, background: "#e8f8f5", color: "#1a9e87", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 12 }}>{i + 1}</div>
                            <textarea value={paso} onChange={e => editarPaso(i, e.target.value)} placeholder={`Paso ${i + 1}...`} rows={2} style={{ flex: 1, padding: "9px 12px", border: "1.5px solid #e2e8f0", borderRadius: 10, fontSize: 14, fontFamily: "inherit", resize: "vertical", outline: "none", boxSizing: "border-box" }} onFocus={e => e.target.style.borderColor = "#2ec4a9"} onBlur={e => e.target.style.borderColor = "#e2e8f0"} />
                            <button type="button" className="btn-peligro" onClick={() => eliminarPaso(i)} style={{ marginTop: 6, padding: "6px 10px" }}>✕</button>
                        </div>
                    ))}
                </div>
                <button type="button" className="btn-primario" onClick={() => onGuardar(form)} disabled={!form.nombre.trim()}>
                    💾 {inicial ? "Actualizar receta" : "Guardar receta"}
                </button>
            </div>
            {modalNuevoIng && <ModalNuevoIngrediente nombreSugerido={modalNuevoIng.nombre} onGuardar={handleGuardarNuevoIng} onCancelar={() => setModalNuevoIng(null)} />}
        </>
    )
}

export default function Recetario() {
    const { recetas, inventario, cargando, guardarReceta, eliminarReceta, agregarIngredienteInventario } = useRecetario()
    ...
    if (cargando) return <div style={{ padding: 40, textAlign: "center", color: "var(--texto-suave)" }}>⏳ Cargando recetario...</div>
    ...
    const imprimir = (lista) => {
        inyectarEstiloImpresion(); setRecetasParaImprimir(lista); setDetalle(null)
        setTimeout(() => { const el = document.getElementById("recetario-print"); if (el) el.style.display = "block"; window.print(); if (el) el.style.display = "none" }, 100)
    }

    const handleGuardar = async (form) => { await guardarReceta(form, editando); setMostrarFormulario(false); setEditando(null) }
    const handleEliminar = async (receta) => { await eliminarReceta(receta); setDetalle(null) }
    const abrirEditar = (receta) => { setEditando(receta); setDetalle(null); setMostrarFormulario(true); window.scrollTo({ top: 0, behavior: "smooth" }) }
    ...
    return (
        <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12, marginBottom: 20 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                    {recetasFiltradas.length > 0 && <button type="button" onClick={() => imprimir(recetasFiltradas)} style={{ all: "unset", cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 6, background: "var(--gris)", color: "var(--texto)", padding: "9px 16px", borderRadius: 10, fontSize: 13, fontWeight: 600, border: "1.5px solid var(--gris-borde)" }}>🖨️ Imprimir ({recetasFiltradas.length})</button>}
            ...
            {recetas.length > 0 && (
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 20 }}>
                    ...
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                        {categorias.map(cat => <button key={cat} type="button" onClick={() => setFiltroCategoria(cat)} style={{ all: "unset", cursor: "pointer", padding: "8px 14px", borderRadius: 20, fontSize: 13, fontWeight: 500, background: filtroCategoria === cat ? "var(--verde-oscuro)" : "var(--gris)", color: filtroCategoria === cat ? "#fff" : "var(--texto)", border: "1.5px solid " + (filtroCategoria === cat ? "var(--verde-oscuro)" : "transparent") }}>{cat}</button>)}
                    </div>
                </div>
            )}
            {recetas.length === 0 && !mostrarFormulario ? (
                <div className="card" style={{ textAlign: "center", padding: "48px 24px" }}>
                    ...
                    <p style={{ color: "var(--texto-suave)", fontSize: 14, marginBottom: 20 }}>Creá tu primera receta con ingredientes y pasos de preparación.</p>
                    <button type="button" className="btn-primario" onClick={() => setMostrarFormulario(true)}>➕ Crear primera receta</button>
                </div>
            ) : recetasFiltradas.length === 0 && recetas.length > 0 ? (
                <p style={{ color: "var(--texto-suave)", textAlign: "center", padding: 32 }}>No hay recetas que coincidan con la búsqueda.</p>
            ) : (
